...
public class QuizController {
    ...
    public ResponseEntity<QuizAttemptResponseDTO> createQuizAttempt(@RequestBody QuizAttemptRequestDTO quizAttemptRequestDTO) {
        logger.info("Creating new quiz attempt for userId: {}, quizId: {}", quizAttemptRequestDTO.getUserId(), quizAttemptRequestDTO.getQuizId());
    ...
    public ResponseEntity<QuizAttemptResponseDTO> getQuizAttemptByUserAndQuiz(@PathVariable String userId, @PathVariable String quizId) {
        logger.info("Fetching quiz attempt for userId: {}, quizId: {}", userId, quizId);
        try {
            Long userIdLong = Long.parseLong(userId); 
            Long quizIdLong = Long.parseLong(quizId);
            logger.debug("Converted userId to Long: {}, quizId to Long: {}", userIdLong, quizIdLong);

            Optional<QuizAttemptResponseDTO> quizAttempt = quizAttemptService.getQuizAttemptByUserIdAndQuizId(userIdLong, quizIdLong);
            if (quizAttempt.isPresent()) {
                logger.info("Found quiz attempt for userId: {} and quizId: {}", userId, quizId);
                return ResponseEntity.ok(quizAttempt.get());
            } else {
                logger.warn("No quiz attempt found for userId: {} and quizId: {}", userId, quizId);
                return ResponseEntity.status(HttpStatus.NOT_FOUND).build();
            }
        } catch (NumberFormatException e) {
            logger.error("Invalid userId or quizId format: userId={}, quizId={}", userId, quizId, e);
    ...
    public ResponseEntity<QuestionAttemptDTO> getQuestionAttempt(@PathVariable Long quizAttemptId, @PathVariable Long questionId) {
        logger.info("Fetching question attempt for quizAttemptId: {}, questionId: {}", quizAttemptId, questionId);
        ...
        return questionAttemptOptional
                .map(questionAttempt -> {
                    logger.info("Found question attempt for quizAttemptId: {}, questionId: {}", quizAttemptId, questionId);
